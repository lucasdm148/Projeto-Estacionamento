...
class App():
    ...
    def menus(self):
        ...
        self.menu_mensalista.add_command(label='Tabela de Mensalistas')
    ...
    def frames(self):
        ...
        self.treewiew = ttk.Treeview(master=self.frame2, columns=('Placa', 'Tipo', 'Marca', 'Cor', 'Hora'),
                                     show='headings')
        ...
        self.treewiew.place(relx=0, rely=0, relwidth=1, relheight=1)
    ...
    def labels(self):
        ...
        self.lb_auxiliar = tk.Label(master=self.Frame1, background='white')
    ...
    def janela_calculadora(self, preco):
        ...
        self.lb_valor_troco.place(relx=0.3, rely=0.45, relwidth=0.25, relheight=0.15)
    ...
    def comando_btn_entrada(self):
        ...
        if placa == '':
            ...
            if mensagem_confirmacao is True:
                ...
                self.cursor.execute("""SELECT Placa FROM mensalista""")
                ...
                if recibo_carimbado is False:
                    if tipo_veiculo == 'Carro':
                        if 0 <= horas <= 1:
                            ...
                            preco = horas * preco_carro
                        self.janela_calculadora(preco)
                    else:
                        if 0 <= horas <= 1:
                            preco = preco_moto
                        else:
                            preco = preco_moto * horas
                        self.janela_calculadora(preco)
                    ...
                    self.e_placa2.delete(first_index=0, last_index=tk.END)
                    self.carimbado.set(False)
                    self.combo_convenio.set('Não Especificado')
    ...
    def comando_calcular(self):
        ...
        self.texto_troco.set('R$ %.2f' % (troco))
    def comando_ok(self):
        self.conectar_banco_de_dados()
        self.cursor.execute("""DELETE FROM patio WHERE PLACA = ?""", (self.placa_saida,))
        self.conn.commit()
    ...
    def janela_inserir_mensalista(self):
        def comando_btn_cadastrar():
            ...
            if placa == '':
                ...
                self.conectar_banco_de_dados()
                self.cursor.execute("""INSERT INTO mensalista (Placa, Nome, Valor) VALUES (?,?,?)""",
                                    (placa, nome, float(mensalidade)))
                self.conn.commit()
                self.desconecta_banco_de_dados()
                messagebox.showwarning('Sucesso', message='Veículo adicionado com sucesso')
